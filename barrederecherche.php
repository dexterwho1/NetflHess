...
<section>
    ...
    <aside class="movieGallery">
        ...
        <?php
        $req = $bdd->prepare('SELECT id, realisateur, image, genre, titre, date_parution FROM film WHERE titre LIKE :titre OR realisateur like :titre');
        $req->execute(array(
            ':titre' => '%' . $_POST['search'] . '%'
        ));

        if($req->rowCount() == 0){
            echo '<p class="resultremove">Aucun résultat</p>';
            echo '<h2 class="resultremove">Pas de soucis nous avons d\'autres bon contenu</h2>';
            $req = $bdd->query('SELECT id, realisateur, image, genre, titre, date_parution FROM film ORDER BY date_parution DESC LIMIT 20');
        }
